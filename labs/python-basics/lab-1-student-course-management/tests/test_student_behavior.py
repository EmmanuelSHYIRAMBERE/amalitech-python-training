...
class TestDunderMethods:
    ...
    def test_hash_consistency(self, undergrad: UndergraduateStudent) -> None:
        assert hash(undergrad) == hash(undergrad)

    def test_equal_students_same_hash(self) -> None:
        s1 = UndergraduateStudent("S001", "Alice", "a@example.com", 1)
        s2 = UndergraduateStudent("S001", "Alice", "a@example.com", 1)
        assert hash(s1) == hash(s2)

    def test_students_usable_in_set(
        self, undergrad: UndergraduateStudent, grad: GraduateStudent
    ) -> None:
        student_set = {undergrad, grad}
        assert len(student_set) == 2

    def test_str_contains_name_and_id(self, undergrad: UndergraduateStudent) -> None:
        s = str(undergrad)
        assert "Alice Johnson" in s
        assert "S001" in s

    def test_repr(self, undergrad: UndergraduateStudent) -> None:
        r = repr(undergrad)
