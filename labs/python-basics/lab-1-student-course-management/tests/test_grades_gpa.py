...
class TestGradesAndGPA:
    def test_gpa_no_grades(self, undergrad: UndergraduateStudent) -> None:
        assert undergrad.calculate_gpa() == 0.0

    def test_gpa_single_grade(self, undergrad: UndergraduateStudent) -> None:
        undergrad.add_grade("CS101", 3.5)
        assert undergrad.calculate_gpa() == pytest.approx(3.5)

    def test_gpa_multiple_courses(self, undergrad: UndergraduateStudent) -> None:
        undergrad.add_grade("CS101", 3.5)
    ...
    def test_invalid_grade_raises(
        self, undergrad: UndergraduateStudent, grade: float
    ) -> None:
        with pytest.raises(ValueError, match="Grade"):
            undergrad.add_grade("CS101", grade)

    @pytest.mark.parametrize("grade", [0.0, 2.0, 4.0])
    def test_boundary_grades_accepted(
        self, undergrad: UndergraduateStudent, grade: float
    ) -> None:
