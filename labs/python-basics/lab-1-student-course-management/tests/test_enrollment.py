...
class TestEnrollment:
    ...
    def test_drop_enrolled_course(self, undergrad: UndergraduateStudent) -> None:
        undergrad.enroll_course("CS101")
        assert undergrad.drop_course("CS101") is True
        assert "CS101" not in undergrad.get_enrolled_courses()

    def test_drop_unenrolled_course_returns_false(
        self, undergrad: UndergraduateStudent
    ) -> None:
        assert undergrad.drop_course("CS999") is False

    def test_enroll_multiple_courses(self, undergrad: UndergraduateStudent) -> None:
